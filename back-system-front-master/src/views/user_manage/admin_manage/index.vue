		...
		<div class="table-top">
			...
			<div class="table-header">
				...
				<div class="button-wrapped">
					<el-button v-if="role === 'admin' || role === 'teacher'" type="primary" @click="openCreate(1)">添加管理员</el-button>
				</div>
			</div>
			<!-- 表格内容 -->
...
<script lang="ts" setup>
	...
	const searchAdmin = async () => {
		tableData.value = await searchUser(adminAccount.value, '管理员')
	...
	const returnAdminListLength = async () => {
		const res = await getAdminListLength('管理员')
	...
	const getFirstPageList = async () => {
		tableData.value = await returnListData(1, '管理员')
	}
	getFirstPageList()
	// 监听换页
	const currentChange = async (value : number) => {
		paginationData.currentPage = value
		tableData.value = await returnListData(paginationData.currentPage, '管理员')
	}
