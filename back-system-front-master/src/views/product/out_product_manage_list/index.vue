		...
		<div class="table-top">
			<div class="table-header">
				<div class="search-wrapped">
					<el-input v-model="productOutId" class="w-50 m-2" size="large" placeholder="输入出库编号进行搜索"
						:prefix-icon="Search" @change="searchProductOutId()" clearable @clear="getFirstPageList" />
				</div>
			</div>
...
<script lang="ts" setup>
	...
	import {
		searchProductForOutId, returnOutProductListData, getOutProductLength
	} from '@/api/product.js'
	import {
		Search
	...
	const currentChange = async (value : number) => {
		...
		tableData.value = await returnOutProductListData(paginationData.currentPage)
	}
